...
class Teleport2(morse.core.actuator.Actuator):
    """ 
    ...
    add_data('pitch', 'Initial robot pitch', "float", 'Rotation of the robot around Y axis, in radian')
    add_data('roll', 'Initial robot roll', "float",
             'Rotation of the robot around X axis, in radian')
    add_data('change', 'Boolean flag of whether a change has occured', "bool", 'logical if change')
    ...
    def default_action(self):
        ...
        change = True
        # try:
        #     change = self.local_data['change']
        # except:
        #     print("teleporting postion")
        #     change = True
        # self.local_data['change'] = True
        if change:
            # New parent position
